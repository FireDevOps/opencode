...
const main = Effect.gen(function* () {
  ...
  const appId = app.isPackaged ? APP_IDS[CHANNEL] : "ai.aog.hub.dev"
  const onboardingTestRoot = ((): string | undefined => {
    if (!TEST_ONBOARDING) return

    const root = join(tmpdir(), `hub-onboarding-${randomUUID()}`)
  ...
  app.setName(app.isPackaged ? APP_NAMES[CHANNEL] : "[AoG] Hub Dev")
  ...
  app.on("second-instance", (_event: Event, argv: string[]) => {
    const urls = argv.filter(
      (arg: string) => arg.startsWith("opencode://") || arg.startsWith("forge://") || arg.startsWith("devhub://") || arg.startsWith("hub://"),
    )
    if (urls.length) {
  ...
  app.setAsDefaultProtocolClient("hub")
  registerRendererProtocol()
  setDockIcon()
